...
def get_home() -> HomeResponse:
    ...
    return HomeResponse(
        ...
        features=[
            FeatureItem(icon="Brain", title="智能刷题", desc="随机抽题 + 自动判分，错题本与间隔复习一体化。"),
            FeatureItem(icon="Target", title="岗位匹配", desc="按学历 / 专业 / 年龄结构化筛选，算出真正能报的岗。"),
            FeatureItem(icon="Mic", title="面试训练", desc="录音转写 + AI 点评，表达力可量化提升。"),
            FeatureItem(icon="Network", title="知识图谱", desc="专业科目知识图谱，强弱项一眼看穿。"),
        ],
        cta_text="开始备考",
        cta_url="/api/home",
    )
